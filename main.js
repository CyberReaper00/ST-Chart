...
function get_file() {
    const user_input = prompt('Enter filename');
...
let datas;
fn.select('#lcl_file', 'lcl_file_element');
fn.select('#server_file', 'server_file_element');

if (vars.lcl_file_element) {
    vars.lcl_file_element.addEventListener('change', async function() {

	const file_contents = await get_file_contents('#lcl_file');

	if (!file_contents) {
	    fn.err('Invalid input. Please enter the values again', 'error');
	    return;
	}

	try {

	    datas = d3.csvParse(file_contents);

	    if (!datas || datas.length === 0) {
		fn.err('No data loaded from system', 'error');
		return;
	    }
	    ...
	    fn.err(`
		Failed to load file: ${filename}.csv
		Error: ${error.message}
	    `, 'error');
	}
    });
...
if (vars.server_file_element) {
    vars.server_file_element.addEventListener('click', async function() {
	...
	try {
	    ...
	    datas = await d3.csv(`${filename}.csv`);

	    if (!datas || datas.length === 0) {
		fn.err(`no data loaded from server`, 'error');
		return;
	    }
...
// ==========>> Getting chart info from the dataset
	...
	if (values.length > 0) {
	    all_vals = all_vals.concat(values);
	    total_months++;
		...
		sh_ra: ${sh_ra}
		-------------------------
		results[month]: ${JSON.stringify(results[month])}
	    `)
	}
    }
