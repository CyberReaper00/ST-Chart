...
class fn {
    ...
    static select(element, new_var, var_obj = vars) {
...
function all_changes() {
    fn.select('.sv_btn', 'save');

    vars.save.addEventListener('click', function() {
...
function get_file_contents(file_input_id) {
	...
	const file_input = document.querySelector(file_input_id);
...
document.querySelector('.file_input').addEventListener('change', function() {
	...
	try {
	    ...
	    fn.err(`datas: ${JSON.stringify(datas)}`);
	    ...
	    fn.err(`init_val: ${JSON.stringify(vars.init_val.value)}`);
	// Placing new data point at the beginning of the dataset
	const data = [first_val, ...datas];
	fn.err(`data: ${JSON.stringify(data)}`);
	...
	for (let i = data.length - 1; i >= data.length - 3; i--) { // Start from the last index
